...
#[tokio::test]
async fn checkpoint_http_requires_independent_roles_and_owns_progress_by_subject() {
    ...
    let page = request(
        ...
        &consumer,
        json!({"contract_version":1,"scope":memory_scope("shared"),"query":{"limit":10}}),
    )
    .await;
    let cursor = page.1["page"]["next_cursor"].clone();
    let body = json!({"scope":memory_scope("shared"),"command":{"contract_version":1,"idempotency_key":"checkpoint","consumer_id":"cache","expected_revision":0,"cursor":cursor}});
    ...
    let read = json!({"contract_version":1,"scope":memory_scope("shared"),"consumer_id":"cache"});
    assert_eq!(
        request(
            &router,
            "POST",
