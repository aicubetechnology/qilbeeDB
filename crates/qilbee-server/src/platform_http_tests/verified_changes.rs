...
#[tokio::test]
async fn verified_memory_change_feed_preserves_resume_fence_and_restart() {
    ...
    assert_eq!(
        request(&router, "POST", "/api/v2/memory/changes", &token, query)
            .await
...
#[tokio::test]
async fn verified_memory_change_feed_isolates_tenants_subjects_grants_and_revoked_credentials() {
    let dir = TempDir::new().unwrap();
    let (router, identity) = app(dir.path());
    let admin = identity.bootstrap_tenant("tenant", "operator").unwrap();
    ...
    for key in [&other, &outside] {
        ...
        assert_eq!(
            request(&router, "POST", "/api/v2/memory/changes", key, q)
                .await
                .0,
            StatusCode::CONFLICT
        );
    ...
    for field in ["project_id", "mission_id", "agent_id"] {
        ...
        assert_eq!(
            request(&router, "POST", "/api/v2/memory/changes", &token, q)
                .await
                .0,
