    ...
    for page in pages:
        ...
        def link(match):
            ...
            if not target.is_relative_to(ROOT) or not target.is_file():
                raise ValueError(f'Unresolved documentation link in {page["source"]}: {destination}')
        ...
        metadata = dict(title=page['title'], description=page['description'], slug='qilbeedb' if page['file']=='index.md' else 'qilbeedb/'+Path(page['file']).stem,
                        section='QilbeeDB', order=page['order'], language='en', release=manifest['release'], status=status,
                        source=page['source'], source_sha256=hashlib.sha256(original.encode()).hexdigest())
...
def main():
    ...
    parser.add_argument('--output', type=Path, required=True, help='Managed QilbeeDB subdirectory under the site app/src/doc')
    parser.add_argument('--status', choices=['unreleased','released'], default='unreleased')
    parser.add_argument('--check', action='store_true', help='Report missing or stale managed files without changing them')
    ...
    for filename in rendered:
        if (args.output/filename).is_symlink(): raise ValueError('Refusing to replace a symlink: '+filename)
    stale=[name for name,body in rendered.items() if not (args.output/name).is_file() or (args.output/name).read_text(encoding='utf-8') != body]
    if args.check:
        if stale:
            print('User documentation needs export: '+', '.join(stale),file=sys.stderr)
            return 1
    ...
    print(f'Exported {len(stale)} updated Markdown guides; {len(rendered)} managed pages. Unrelated files preserved.')
    return 0
if __name__=='__main__':
    try:sys.exit(main())
    except (OSError, ValueError, KeyError) as error:
        print('Documentation export failed: '+str(error),file=sys.stderr);sys.exit(1)
